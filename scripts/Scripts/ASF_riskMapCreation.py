...
def checkFileAndFolder(workingDirectory):  # Checking the required files and directories
    ...
    try:
        file = open(sirModelPath + '/simulatingMovement.R', 'r')
        print('{} is checked.'.format(sirModelPath + '/simulatingMovement.R'))  
    except IOError:
        print('Cannot find {}\nProgram will close.'.format(sirModelPath + '/simulatingMovement.R'))
        sys.exit(1)
...
def simulateEmove_modelProcessing(sirModelPath,weekFolder,realEmoveCSVFile,initialSD, seed): # Run emove simulating 
    ...
    print('Running R model => START')
    try:
        subprocess.call ("Rscript --vanilla {0}/simulatingMovement.R {1} {2} {3} {4}".format(sirModelPath,weekFolder,realEmoveCSVFile,initialSD,seed), shell=True)
        print('Running R model => DONE')
    except:
        print('Running R model => FAILED') 
