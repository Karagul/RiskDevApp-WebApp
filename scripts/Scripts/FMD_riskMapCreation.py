...
def simulateEmove_modelProcessing(sirModelPath,weekFolder,realEmoveCSVFile,initialSD, seed): # Run emove simulating 
    ...
    print('Running R model => START')
    try:
        subprocess.call ("Rscript --vanilla {0}/FMD_simulatingMovement.R {1} {2} {3} {4}".format(sirModelPath,weekFolder,realEmoveCSVFile,initialSD,seed), shell=True)
        print('Running R model => DONE')
    except:
        print('Running R model => FAILED') 
...
if __name__ == "__main__": 
    ...
    Model100Iterations(workingDirectory=args.workingDirect, maxIterations = 100, cleanUpPeriod=args.cleanUpPeriod, beta=args.beta, gamma=args.gamma,  sigma=args.sigma, maxLoop = 40, epidemicType='FMD')
