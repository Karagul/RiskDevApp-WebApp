...
def file_process_population(filepath, result_type, result_year, subdistrict_list):
    ...
    try:
        ...
        with db_connection.cursor() as db_cursor:
            ...
            animal_master_table = {x["animal_code"]: x["animal_name"] for x in animal_master_data}
            ...
            for row_index, row_data in file_output_dataframe.iterrows():
                ...
                file_output_dataframe.at[row_index, "Province"] = query_result_ressub[0]["province_name_th"]
                file_output_dataframe.at[row_index, "District"] = query_result_ressub[0]["district_name_th"].replace("กิ่งอำเภอ", "")
                file_output_dataframe.at[row_index, "Subdistrict"] = query_result_ressub[0]["subdistrict_name_th"]
                file_output_dataframe.at[row_index, "RiskLevel"] = query_result_ressub[0]["risk_level_final"]
                file_output_dataframe.at[row_index, "RiskNormDist"] = translate_normdist_desc(query_result_ressub[0]["risk_level_normdist"])
                file_output_dataframe.at[row_index, "SourceSubdistrict"] = query_result_ressub[0]["starting_subdistrict_code"]
                ...
                """.format(subdistrict_code = query_result_ressub[0]["starting_subdistrict_code"])
                ...
                file_output_dataframe.at[row_index, "SourceSubdistrict"] = "จ.{province_name} อ.{district_name} ต.{subdistrict_name}".format(
                    province_name = query_result_stasub[0]["province_name_th"],
                    district_name = query_result_stasub[0]["district_name_th"].replace("กิ่งตำบล", ""),
                    subdistrict_name = query_result_stasub[0]["subdistrict_name_th"]
                )
        ...
        file_output_dataframe = file_output_dataframe.sort_values(["Province", "District", "Subdistrict", "RiskLevel", "RiskNormDist", "AnimalTotal"], ascending=[True, True, True, False, False])
...
def main(result_type, result_year, email_recipient, subdistrict_list):
    ...
    if result_type == "ASF":
        file_input_location = "~/Repositories/RiskDevApp-WebApp/results/Population_nipah_" + result_year + ".csv"
    else:
