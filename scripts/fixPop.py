...
def repairMissPopInSD(missedSD , popData): # Repai data in the SD that not have animal population
    for sd in missedSD:
        dCode = sd[0][:3]
        # print(dCode)
...
def main(emoveDataPath, popDataPath, savePath): # Main
    if emoveDataPath != None:
        emoveData = loadCSV(emoveDataPath + '/DataMart_Emove.csv', 1)                  # Read emovement csv file
    else:
        emoveData = loadCSV('DataMart_Emove.csv', 1)                                   # Read emovement csv file
    if popDataPath != None:
        popData = loadCSV(popDataPath + '/Population.csv' , 1)                         # Read animal population csv file  
    else:    
        popData = loadCSV('Population.csv' , 1)                                        # Read animal population csv file 
    ...
    csvFile = ''
    if savePath != None:
        csvFile = savePath + '/Pig_POP.csv'                                            # Read emovement csv file
    else:
        csvFile = 'Pig_POP.csv'                                                        # Read emovement csv file
...
if __name__ == "__main__": 
    ...
    parser.add_argument('--emovePath', metavar='',type=str, required=False,
                        help='the emovement path')
    parser.add_argument('--popPath', metavar='',type=str, required=False,
                        help='the population data path')
    parser.add_argument('--savePath', metavar='',type=str, required=False,
