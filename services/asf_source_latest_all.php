...
if(isset($_GET["return_type"])) echo asf_source_latest_all(true);

function asf_source_latest_all($bool_return_json) {
    global $db_conn;

    $asf_source_query = $db_conn->prepare("SELECT DISTINCT result_for_year, starting_subdistrict_code, subdistrict_name_th, 
                                              ...
                                              WHERE execute_type_name = 'ASF'
                                                AND result_for_year = (SELECT MAX(result_for_year) FROM execute_result WHERE execute_type_name = 'ASF')
