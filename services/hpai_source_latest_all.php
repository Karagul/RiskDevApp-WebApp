...
if(isset($_GET["return_type"])) echo hpai_source_latest_all(true);

function hpai_source_latest_all($bool_return_json) {
    global $db_conn;

    $hpai_source_query = $db_conn->prepare("SELECT DISTINCT result_for_year, starting_subdistrict_code, subdistrict_name_th, 
                                              ...
                                              WHERE execute_type_name = 'HPAI'
                                                AND result_for_year = (SELECT MAX(result_for_year) FROM execute_result WHERE execute_type_name = 'hpai')
